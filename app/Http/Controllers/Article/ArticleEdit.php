...
class ArticleEdit extends ArticleBase
{
    public function index()
    {
        ...
        //获取文章主要信息
        $articleInfo = (new Article())->getOne(['fields'=>['id','title','introduction','pic_url'],'where'=>['id'=>$id,'status !='=>-1]]);
        if (!$articleInfo) {
            return ['code'=>-1,'没有找到该文章或者该文章已经被删除'];
        }
        //文章封面
        $articleInfo['pic_url_show'] = $fileHost.$articleInfo['pic_url'];
        //获取文章子内容
        $articleInfo['content'] = [];
        ...
        if ($articleContent) {
            foreach ($articleContent as &$articleContentItem) {
                if ($articleContentItem['type'] == 2) {
                    $articleContentItem['pic_url_show'] = $fileHost.$articleContentItem['pic_url'];
                }
            }
        }
